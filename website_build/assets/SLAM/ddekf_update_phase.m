function [estimate_next, covariance_sqrt] = ddekf_update_phase(R_root,P_root,Cjacobian,Cmeasurement,estimate,measurement)
...
%	covariance_sqrt*(covariance_sqrt')
	...
	measurement_count = size(Cjacobian,1); 
	state_count = size(Cjacobian,2);
	
	tmp = zeros(state_count+measurement_count, state_count+measurement_count);
	tmp(1:measurement_count,1:measurement_count) = R_root;
	tmp(1:measurement_count,(measurement_count+1):end) = Cjacobian*P_root;
	...
	%Kalman_gain = Y*inv(X);
	estimate_next = estimate + Y*(X\(measurement - Cmeasurement));
	covariance_sqrt = Z;
end
